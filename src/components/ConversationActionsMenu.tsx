...
import {
    ...
    AlertDialogTitle,
    AlertDialogTrigger,
  } from '@/components/ui/AlertDialog.tsx'; // Import AlertDialog
  import {
    DropdownMenu,
    DropdownMenuContent,
    DropdownMenuItem,
...
importChatsAtom, // Import new atom
    ...
    togglePinChatAtom,
  } from '@/store/atoms.ts'; // Import necessary atoms
  ...
  // TODO: Define these actions in atoms.ts
  const forkChatAtom = atom(null, (get, set, chatId: string) => { console.warn('forkChatAtom not implemented'); /* Implement logic in atoms.ts */ });
  const deleteChatsOlderThanAtom = atom(null, (get, set, chatId: string) => { console.warn('deleteChatsOlderThanAtom not implemented'); /* Implement logic in atoms.ts */ });
  const deleteChatsNewerThanAtom = atom(null, (get, set, chatId: string) => { console.warn('deleteChatsNewerThanAtom not implemented'); /* Implement logic in atoms.ts */ });
  const importChatsAtom = atom(null, (get, set, chats: Chat[]) => { console.warn('importChatsAtom not implemented'); /* Implement logic in atoms.ts */ });
  // --- End TODO ---
  ...
  export const ConversationActionsMenu: React.FC = () => {
   ...
   const importChats = useSetAtom(importChatsAtom);
    ...
    const canDeleteBelow = isChatActive && activeChatIndex < sortedChats.length - 1 && activeChatIndex !== -1;
  
    const hasUnpinned = useMemo(() => chatList.some(c => !c.isPinned), [chatList]);
    ...
    const handlePinToggle = () => {
      if (activeChat) {
        ...
        toast.success(isChatPinned ? 'Conversation unpinned' : 'Conversation pinned');
      }
    };
    ...
    // --- Import/Export Handlers ---
     ...
     const handleFileImport = async (event: React.ChangeEvent<HTMLInputElement>) => {
         ...
         try {
             ...
             toast.success(`${importedData.length} conversation(s) imported successfully!`);
     ...
     const handleExportCurrent = () => {
          ...
          downloadJson([activeChat], `conversation_${activeChat.name.replace(/\s+/g, '_')}.json`); // Use helper
      ...
      <>
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end" className="w-56">
            ...
            <DropdownMenuItem onClick={createNewChat}> {/* Direct call */}
            ...
            <AlertDialog>
               <AlertDialogTrigger asChild>
                    <DropdownMenuItem
                      ...
                      className={(!isChatActive || !canDeleteBelow) ? 'cursor-not-allowed text-muted-foreground/50 hover:bg-transparent' : ''}
                     >
               ...
               <AlertDialogContent>
                    <AlertDialogHeader>
                      ...
                      <AlertDialogDescription>
                         This action cannot be undone. This will permanently delete all conversations created before the current one (excluding pinned conversations).
                      </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
             ...
             <AlertDialog>
                 <AlertDialogTrigger asChild>
                      <DropdownMenuItem
                          ...
                          className={(!isChatActive || !canDeleteAbove) ? 'cursor-not-allowed text-muted-foreground/50 hover:bg-transparent' : ''}
                      >
                  ...
                  <AlertDialogContent>
                     <AlertDialogHeader>
                       <AlertDialogTitle>Are you absolutely sure?</AlertDialogTitle>
                       <AlertDialogDescription>
                         This action cannot be undone. This will permanently delete all conversations created after the current one (excluding pinned conversations).
                       </AlertDialogDescription>
                     </AlertDialogHeader>
                     <AlertDialogFooter>
                       <AlertDialogCancel>Cancel</AlertDialogCancel>
             ...
             <AlertDialog>
                  <AlertDialogTrigger asChild>
                     <DropdownMenuItem
                         ...
                         className={!hasUnpinned ? 'cursor-not-allowed text-muted-foreground/50 hover:bg-transparent' : ''}
                     >
                  ...
                  <AlertDialogContent>
                      <AlertDialogHeader>
                        <AlertDialogTitle>Are you absolutely sure?</AlertDialogTitle>
                        <AlertDialogDescription>
                          This action cannot be undone. This will permanently delete all conversations that are not pinned.
                        </AlertDialogDescription>
                      </AlertDialogHeader>
                      <AlertDialogFooter>
                        <AlertDialogCancel>Cancel</AlertDialogCancel>
               ...
               <AlertDialog>
                   <AlertDialogTrigger asChild>
                      <DropdownMenuItem
                           ...
                           className={!isChatActive ? 'cursor-not-allowed text-muted-foreground/50 hover:bg-transparent' : ''}
                      >
                   ...
                   <AlertDialogContent>
                       <AlertDialogHeader>
                         <AlertDialogTitle>Are you absolutely sure?</AlertDialogTitle>
                         <AlertDialogDescription>
                           This action cannot be undone. This will permanently delete the current conversation and all its messages.
                         </AlertDialogDescription>
                       </AlertDialogHeader>
                       <AlertDialogFooter>
                         <AlertDialogCancel>Cancel</AlertDialogCancel>
